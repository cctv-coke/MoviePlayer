...
class FavoritesPage(QWidget):
    ...
    def _init_ui(self):
        ...
        clear_btn = QPushButton("🗑️ 清空收藏")
        clear_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        clear_btn.clicked.connect(self._clear_favorites)
    ...
    def _load_favorites(self):
        ...
        favorites = self.main_window.config.get_favorites()
        if not favorites:
            empty_label = QLabel("📭 还没有收藏任何内容")
    ...
    def _clear_favorites(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self.main_window.config._save_favorites([])
            self._load_favorites()
